...
% Se necesitan los archivos que contengan el indice de refraccion del Si
% cristalino para distintas longitudes de onda.
...
% REAL n OF Si
data = readmatrix('n_cSi.txt');
wl_n= data(:,1) * 1000 * nanometers;
nSi = data(:,2);

% IMAGINARY n OF Si
data = readmatrix('k_cSi.txt');
wl_k = data(:,1) * 1000 * nanometers;
kSi = data(:,2);  
...
if length(wavelength) ~= 1
    figure(1)
    plot(wavelength(16:46), R, 'o-', 'LineWidth', 1); hold on;
    plot(wavelength(16:46), T, 'o-', 'LineWidth', 1);
