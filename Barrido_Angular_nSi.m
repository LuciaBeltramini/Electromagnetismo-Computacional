...
% electromagnetica al cambiar de medio, siendo este SILICIO NANOPOROSO. 
...
alfa2 = 80 * degrees;
Delta = 5 * degrees;
alfa = alfa1:Delta:alfa2;
barrido_a = true;

% POLARIZATION ('E' = s // 'H' = p )
MODE  = 'H';      
...
%% SWEEPS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if barrido_a == false & barrido_w == true
    for lam0 = wavelength
        fprintf('------------------ LAMDA [nm] = %.1f ------------------\n',  lam0/nanometers); 
        t_rep = [];
        r_rep = [];
        ...
        for times=1:rep
            ...
            % EXTRACT MATERIAL PROPERTIES AT EACH POINT
            e_ref = ER2(1,1) * e0;
            e_trn = ER2(1,Ny2) * e0;
            omega = 2 * pi * f0;
            ...
            if MODE == 'E'
               % TOTAL, SOURCE AND SCATTERED FIELDS
                ...
                Sy_inc_line  = 0.5*real( Ez_inc(:, y_ref)  .* conj(Hx_inc(:,  y_ref)) );  
                Sy_ref_line  = 0.5*real( Ez_scat(:,y_ref)  .* conj(Hx_scat(:, y_ref)) );
                Sy_trn_line  = 0.5*real( Ez_tot(:, y_trn)  .* conj(Hx_tot(:,  y_trn)) ); 
    
            elseif MODE == 'H'
                % TOTAL, SOURCE AND SCATTERED FIELDS
                Hz_tot  = f;        
                Hz_inc  = fsrc;     
                Hz_scat = Hz_tot - Hz_inc;
                
                % PARTIAL DERIVATIVES OF Hz
                [Hzy_tot, Hzx_tot]   = gradient(Hz_tot, dy, dx); 
                [Hzy_inc, Hzx_inc]   = gradient(Hz_inc, dy, dx);
                [Hzy_scat,Hzx_scat]  = gradient(Hz_scat,dy, dx);
                
                %CALCULATE ELECTRIC FIELDS
                Ex_tot  =  (1./(1i*omega * e_trn)) .* Hzy_tot;
                Ex_inc  =  (1./(1i*omega * e_trn)) .* Hzy_inc;
                Ex_scat =  (1./(1i*omega * e_ref)) .* Hzy_scat;
    
                % POYTING FLUX IN EACH ZI
                Sy_inc_line  = -0.5*real( Ex_inc(:, y_ref)  .* conj(Hz_inc(:,  y_ref)) );  
                Sy_ref_line  = -0.5*real( Ex_scat(:,y_ref)  .* conj(Hz_scat(:, y_ref)) );
                Sy_trn_line  = -0.5*real( Ex_tot(:, y_trn)  .* conj(Hz_tot(:,  y_trn)) );
            end
    ...
    if length(wavelength) ~= 1
        figure(10)
        hold on
        plot(wavelength * 1/nanometers, R*100, '.-', 'LineWidth', 1,'Color',[0.65 0 1]); hold on;
...
elseif barrido_a == true & barrido_w == false
    for theta = alfa
        fprintf('------------------ Angulo de incidencia = %.1f ------------------\n',  theta/degrees); 
        t_rep = [];
        r_rep = [];
        a_rep = [];
    ...
    if length(alfa) ~= 1
        figure(11)
        hold on
        plot(alfa/degrees, R*100, '.-', 'LineWidth', 1,'Color',[0.65 0 1]); hold on;
        plot(alfa/degrees, T*100, '.-', 'LineWidth', 1,'Color',[0 0.75 0.75]); hold on;
        plot(alfa/degrees, AA*100,'.-', 'LineWidth', 1,'Color',[1 0 1])
        xlabel('Angulo de incidencia [deg]');      
        ylabel('R y T porcentual'); 
        title('Reflectancia y Transmitancia en funcion del angulo de incidencia');
        grid on;
        ylim([0 105]);
        legend({'R (Reflectancia)','T (Transmitancia)', 'A (Absrobancia)'}, 'Location','best');   
    end
    ...
    error('No se puede hacer un barrido angular y en longitud de onda al mismo tiempo');
end
